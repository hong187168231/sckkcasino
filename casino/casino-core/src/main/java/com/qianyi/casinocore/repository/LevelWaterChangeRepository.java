...
public interface LevelWaterChangeRepository extends JpaRepository<LevelWaterChange,Long>, JpaSpecificationExecutor<LevelWaterChange> {
    ...
    @Query(value = "select ifnull(sum(lwc.bet_water),0)  from level_water_change lwc WHERE lwc.user_id = ?1 and date_sub(curdate(), INTERVAL 10 DAY) <= date(`create_time`) ",nativeQuery = true)
