...
public interface CompanyProxyMonthRepository extends JpaRepository<CompanyProxyMonth,Long>, JpaSpecificationExecutor<CompanyProxyMonth> {
    ...
    @Query(value = "select user_id,max(proxy_role) proxy_role ,\n" +
            ...
            "max(substr('2021-10-28 00:00:00',1,10)) statics_times \n" +
            "sum(player_num) player_num ,sum(group_bet_amount) group_bet_amount ,max(profit_level) profit_level k\n" +
    ...
    List<CompanyProxyMonth> queryMonthData(String startTime, String endTime);
}
