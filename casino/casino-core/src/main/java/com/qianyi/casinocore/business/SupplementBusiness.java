...
public class SupplementBusiness {
        ...
        while (true) {
            try {
                ...
                PublicWMApi.ResponseEntity entity =
                    wmApi.getMemberTradeReport(thirdAccount, null, orderNo, null, null, null);
                if (entity == null) {
                ...
                if (entity.getErrorCode() == 107) {
                    ...
                    if (errorOrder.getType() == AccountChangeEnum.WM_IN.getType()) {
                        ...
                        Integer count = updateErrorOrderStatus(errorOrder, "自动补单成功，补单金额:"
                            + errorOrder.getMoney().stripTrailingZeros().toPlainString() + ",转入WM时加点失败,加回本地额度");
                        if (count > 0) {
                            // 加回额度
                ...
                if (entity.getErrorCode() == 0) {
                    ...
                    for (WmMemberTradeReportVo vo : records) {
                        if (thirdAccount.equals(vo.getUser()) && orderNo.equals(vo.getOrdernum())) {
                            ...
                            } else if (vo.getOp_code() == 122
                                && errorOrder.getType() == AccountChangeEnum.RECOVERY.getType()) {
                                // 以WM额度为准
                                BigDecimal money = new BigDecimal(vo.getMoney()).abs();
                                // 更新错误订单表状态
                                Integer count = updateErrorOrderStatus(errorOrder,
                                    "自动补单成功，补单金额:" + money.stripTrailingZeros().toPlainString() + ",转出WM时扣点成功,加回本地额度");
                                if (count > 0) {
                                    // 加回额度
        ...
        while (true) {
            try {
                ...
                PublicGoldenFApi.ResponseEntity playerTransactionRecord =
                    goldenFApi.getPlayerTransactionRecord(thirdAccount, time, time, walletCode, orderNo, null);
                ...
                if (!ObjectUtils.isEmpty(errorCode)) {
                    ...
                    if ("9402".equals(errorCode)) {
                        ...
                        if (orderType == AccountChangeEnum.PG_CQ9_IN.getType()
                            || orderType == AccountChangeEnum.SABASPORT_IN.getType()) {
                            // 更新错误订单表状态
                            Integer count = updateErrorOrderStatus(errorOrder,
                                "自动补单成功，补单金额:" + errorOrder.getMoney().stripTrailingZeros().toPlainString() + ",转入"
                                    + errorOrder.getPlatform() + "时加点失败,加回本地额度");
                            if (count > 0) {
                                // 加回额度
                                addMoney(errorOrder.getUserId(), errorOrder.getMoney());
                                // 记录账变
                        ...
                        } else if (orderType == AccountChangeEnum.PG_CQ9_OUT.getType()
                            || orderType == AccountChangeEnum.SABASPORT_OUT.getType()) {
                            // 转出goldenF时，是先扣减goldenF的钱再加回本地，goldenF查询无记录说明没有扣点成功，本地也不用把钱加回来,更新状态就行
                            updateErrorOrderStatus(errorOrder,
                                "自动补单成功，补单金额:0,转出" + errorOrder.getPlatform() + "时扣点失败,额度未丢失");
                        }
                        break;
                    } else {
                        log.error("goldenf远程确认转账记录异常，errorOrder:{},playerTransactionRecord={}", errorOrder.toString(),
                            playerTransactionRecord.toString());
                        continue;
                    }
                }
                ...
                if (translogs.size() > 0) {
                    ...
                    if (orderType == AccountChangeEnum.PG_CQ9_IN.getType()
                        ...
                        // 更新错误订单表状态
                        updateErrorOrderStatus(errorOrder,
                            "自动补单成功，补单金额:0,转入" + errorOrder.getPlatform() + "时加点成功,额度未丢失");
                    } else if (orderType == AccountChangeEnum.PG_CQ9_OUT.getType()
                        || orderType == AccountChangeEnum.SABASPORT_OUT.getType()) {
                        ...
                        // 更新错误订单表状态
                        Integer count = updateErrorOrderStatus(errorOrder,
                            "自动补单成功，补单金额:" + money.stripTrailingZeros().toPlainString() + ",转出"
                                + errorOrder.getPlatform() + "时扣点成功,加回本地额度");
                        if (count > 0) {
                            // 加回额度
                            addMoney(errorOrder.getUserId(), money);
                            // 记录账变
            ...
            try {
                ...
                if ("1017".equals(status)) {
                    ...
                    if (orderType == AccountChangeEnum.AE_IN.getType()) {
                        // 更新错误订单表状态
                        Integer count = updateErrorOrderStatus(errorOrder,
                            "自动补单成功，补单金额:" + money.stripTrailingZeros().toPlainString() + ",转入AE时加点失败,加回本地额度");
                        if (count > 0) {
                            // 加回额度
                            addMoney(errorOrder.getUserId(), money);
                            // 记录账变
                ...
                if (PublicAeApi.SUCCESS_CODE.equals(status)) {
                    ...
                    if (txStatus == 1) {
                        ...
                        if (orderType == AccountChangeEnum.AE_IN.getType()) {
                            ...
                            Integer count = updateErrorOrderStatus(errorOrder,
                                "自动补单成功，补单金额:" + money.stripTrailingZeros().toPlainString() + ",转出AE时扣点成功,加回本地额度");
                            if (count > 0) {
                                // 加回额度
                                addMoney(errorOrder.getUserId(), money);
                                // 记录账变
    ...
    public void addMoney(Long userId, BigDecimal money) {
        ...
        try {
            ...
            log.error("订单自动补单出现异常userId{}money{} {}", userId, money, e.getMessage());
        } finally {
            // 释放锁
            ...
            try {
                ...
                if ("16".equals(errorCode)) { // 转账订单不存在
                    ...
                    if (orderType == AccountChangeEnum.VNC_IN.getType()) {
                        // 更新错误订单表状态
                        Integer count = updateErrorOrderStatus(errorOrder,
                            "自动补单成功，补单金额:" + money.stripTrailingZeros().toPlainString() + ",转入VNC时加点失败,加回本地额度");
                        if (count > 0) {
                            // 加回额度
                            addMoney(errorOrder.getUserId(), money);
                            // 记录账变
                    ...
                    for (TransferVo vo : records) {
                        if (vncAccount.equals(vo.getUserName()) && orderNo.equals(vo.getOrderNo())) {
                            ...
                            if (vo.getOrderType() == 8 && errorOrder.getType() == AccountChangeEnum.VNC_IN.getType()) {
                                ...
                                // 转出WM时，三方先扣减，确认三方扣点成功加回本地余额
                            } else if (vo.getOrderType() == 9
                                && errorOrder.getType() == AccountChangeEnum.VNC_OUT.getType()) {
                                // 以WM额度为准
                                BigDecimal balance = vo.getMoney();
                                // 更新错误订单表状态
                                Integer count = updateErrorOrderStatus(errorOrder, "自动补单成功，补单金额:"
                                    + balance.stripTrailingZeros().toPlainString() + ",转出WM时扣点成功,加回本地额度");
                                if (count > 0) {
                                    // 加回额度
        ...
        while (true) {
            try {
                ...
                if (errorOrder.getType() == AccountChangeEnum.DMC_IN.getType()) {
                    ...
                    if (!flag) {
                        Integer count = updateErrorOrderStatus(errorOrder, "自动补单成功，补单金额:"
                            + errorOrder.getMoney().stripTrailingZeros().toPlainString() + ",转入DMC时加点失败,加回本地额度");
                        if (count > 0) {
                            // 加回额度
                            addMoney(errorOrder.getUserId(), errorOrder.getMoney());
                            // 记录账变
            ...
            try {
                ...
                if ("324".equals(errorCode)) { // 转账订单不存在
                    ...
                    if (orderType == AccountChangeEnum.DG_IN.getType()) {
                        // 更新错误订单表状态
                        Integer count = updateErrorOrderStatus(errorOrder,
                            "自动补单成功，补单金额:" + money.stripTrailingZeros().toPlainString() + ",转入DG时加点失败,加回本地额度");
                        if (count > 0) {
                            // 加回额度
                            addMoney(errorOrder.getUserId(), money);
                            // 记录账变
    ...
    // Thread.sleep(30 * 1000);
    // //查询转账记录
    // PublicWMApi.ResponseEntity entity = publicLottoApi.getMemberTradeReport(thirdAccount, null, orderNo, null, null,
    // null);
    // if (entity == null) {
    // log.error("查询WM交易记录时远程请求异常");
    // continue;
    ...
    // BigDecimal money = new BigDecimal(vo.getMoney()).abs();
    // //更新错误订单表状态
    // Integer count = updateErrorOrderStatus(errorOrder, "自动补单成功，补单金额:" + money.stripTrailingZeros().toPlainString() +
    // ",转出WM时扣点成功,加回本地额度");
    // if (count > 0) {
    // //加回额度
    // addMoney(errorOrder.getUserId(), money);
