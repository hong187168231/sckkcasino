...
public interface WithdrawOrderRepository extends JpaRepository<WithdrawOrder,Long>, JpaSpecificationExecutor<WithdrawOrder> {
    ...
    @Query(value = "select * from withdraw_order u where u.id = ?",nativeQuery = true)
    WithdrawOrder findWithdrawOrderById(Long id);
    ...
    @Modifying
    @Query(value = "update withdraw_order w set w.withdraw_time = w.update_time where w.withdraw_time is null ",nativeQuery = true)
    void updateWithdrawWithdrawTime();
}
