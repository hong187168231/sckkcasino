...
public interface GameRecordDMCRepository extends JpaRepository<GameRecordDMC, Long>, JpaSpecificationExecutor<GameRecordDMC> {

    GameRecordDMC findByEnterpriseIdAndBetOrderNo(String enterpriseId, String betOrderNo);

    @Modifying(clearAutomatically = true)
    @Query("update GameRecordDMC u set u.codeNumStatus=?2 where u.id=?1")
    void updateCodeNumStatus(Long id, Integer codeNumStatus);

    @Modifying(clearAutomatically = true)
    @Query("update GameRecordDMC u set u.extractStatus=?2 where u.id=?1")
    void updateExtractStatus(Long id, Integer status);

    @Modifying(clearAutomatically = true)
    @Query("update GameRecordDMC u set u.washCodeStatus=?2 where u.id=?1")
    void updateWashCodeStatus(Long id, Integer washCodeStatus);

    @Modifying(clearAutomatically = true)
    @Query("update GameRecordDMC u set u.levelWaterStatus=?2 where u.id=?1")
    void updateLevelWaterStatus(Long id, Integer levelWaterStatus);

    @Modifying(clearAutomatically = true)
    @Query("update GameRecordDMC u set u.rebateStatus=?2 where u.id=?1")
    void updateRebateStatus(Long id, Integer rebateStatus);

    @Modifying(clearAutomatically = true)
    @Query("update GameRecordDMC u set u.gameRecordStatus=?2 where u.id=?1")
    ...
    @Query(value = "select MAX(g.id) maxId,ifnull(SUM(d.user_amount), 0 ) as user_amount,ifnull(SUM(d.surplus_amount), 0 ) as surplus_amount,LEFT(g.settle_time,?2) set_time,ifnull(g.first_proxy,0) first_proxy,\n"
        ...
        + "         ifnull(SUM(w.amount),0) amount from game_record_dmc g left join "
