...
public interface ProxyGameRecordReportRepository extends JpaRepository<ProxyGameRecordReport,Long>,
    ...
    @Query(value = "SELECT ifnull( grd.first_proxy, 0 ) first_proxy,ifnull( grd.second_proxy, 0 ) second_proxy,ifnull( grd.third_proxy, 0 ) third_proxy,"
        ...
        + "ifnull( sum( win_money ), 0 )- ifnull( sum( real_money ), 0 )  win_loss FROM game_record_dg grd WHERE bet_time BETWEEN ?1 AND ?2 group by user_id ;",nativeQuery = true)
    ...
    @Query(value = "SELECT ifnull( grd.first_proxy, 0 ) first_proxy,ifnull( grd.second_proxy, 0 ) second_proxy,ifnull( grd.third_proxy, 0 ) third_proxy,"
            + "user_id user_id,count(1) num,ifnull( sum( bet_amount ), 0 ) bet_amount,ifnull( sum( valid_bet_amount ), 0 ) validbet,"
            + "ifnull( sum( net_amount ), 0 )  win_loss FROM game_record_obzr grd WHERE bet_time BETWEEN ?1 AND ?2 group by user_id ;",nativeQuery = true)
