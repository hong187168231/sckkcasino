...
public interface ProxyGameRecordReportRepository extends JpaRepository<ProxyGameRecordReport,Long>,
    ...
    @Query(value = "SELECT a.user_id user_id,a.first_proxy first_proxy,a.second_proxy second_proxy,a.third_proxy third_proxy,"
        ...
        + "ON u.id = goldenf_t.user_id LEFT JOIN (select off.user_id user_id,off.vendor_code vendor_code,count( DISTINCT sk.bet_id ) num,SUM( sk.bet_amount ) bet_amount,"
        + "SUM( sk.bet_amount ) validbet,sum(off.win_amount)-sum( sk.bet_amount ) win_loss FROM (select bet_id bet_id,user_id user_id,vendor_code,SUM(win_amount) win_amount "
        + "from game_record_goldenf t1 where t1.vendor_code = 'SABASPORT' AND t1.trans_type = 'Payoff' AND t1.create_at_str BETWEEN ?1 and ?2 "
        + "GROUP BY t1.bet_id) off LEFT JOIN (SELECT bet_amount, bet_id FROM game_record_goldenf WHERE vendor_code = 'SABASPORT' AND trans_type = 'Stake') sk ON off.bet_id = sk.bet_id GROUP BY user_id) "
