...
public interface ProxyGameRecordReportRepository extends JpaRepository<ProxyGameRecordReport,Long>,
    ...
    @Query(value = "SELECT a.user_id user_id,a.first_proxy first_proxy,a.second_proxy second_proxy,a.third_proxy third_proxy,"
        ...
        + "bet_id FROM game_record_goldenf WHERE vendor_code = 'SABASPORT' AND trans_type = 'Stake' ) sk ON off.bet_id = sk.bet_id LEFT JOIN game_record_goldenf t3 ON off.bet_id = t3.bet_id "
        + "AND t3.trans_type = 'cancelPayoff' GROUP BY off.user_id) "
