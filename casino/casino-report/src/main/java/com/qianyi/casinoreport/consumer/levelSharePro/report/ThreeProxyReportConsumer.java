...
@Slf4j
//@RabbitListener(queues = RabbitMqConstants.THREE_REPORT_PROFIT_QUEUE)
@Component
public class ThreeProxyReportConsumer {
    ...
    public void process(ShareProfitBO shareProfitBO, Channel channel, Message message) throws IOException {
        log.info("THREE-start 总报表-游戏id:{},代理线报表分润队列：{}",shareProfitBO.getRecordId(),shareProfitBO);
        try {
            //进行总报表处理
            levelproxyReportBusiness.processReport(shareProfitBO);
        } catch (Exception e) {
            log.error("THREE 总报表-分润总报表处理执行异常:"+ e);
        }finally {
            channel.basicAck(message.getMessageProperties().getDeliveryTag(),false);
        }
        log.info("THREE-end 总报表-游戏id:{},代理线报表分润队列：{}",shareProfitBO.getRecordId(),shareProfitBO);
    }
}
