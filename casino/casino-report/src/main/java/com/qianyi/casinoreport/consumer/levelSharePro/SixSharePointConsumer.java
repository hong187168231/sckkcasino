...
@Slf4j
@RabbitListener(queues = RabbitMqConstants.SIX_SHAREPROFIT_DIRECTQUEUE)
@Component
public class SixSharePointConsumer {
    ...
    public void process(ShareProfitBO shareProfitBO, Channel channel, Message message) throws IOException {
        log.info("SIX-start 游戏id:{},代理线分润消息队列：{}",shareProfitBO.getRecordId(),shareProfitBO);
        try {
        ...
        log.info("SIX-end 游戏id:{},代理线分润消息队列：{}",shareProfitBO.getRecordId(),shareProfitBO);
    }
}
