...
public class AuthenticationInteceptor extends AbstractAuthenticationInteceptor {
    ...
    public JjwtUtil.Token refreshJwtToken(String token) {
        ...
        synchronized (token.intern()) {
            //多个请求只有一个去刷新token
            Object redisToken = redisUtil.get(Constants.REDIS_TOKEN + authId + "admin");
            ...
            //获取新token
            String refreshToken = JjwtUtil.refreshToken(token, user.getPassWord(), Constants.WEB_REFRESH_TTL, Constants.CASINO_ADMIN);
            ...
            if (redisJwtToken != null && (token.equals(redisJwtToken.getOldToken()) || token.equals(redisJwtToken.getNewToken()))) {
                redisUtil.set(Constants.REDIS_TOKEN + authId + "admin", jwtTiken, Constants.WEB_REFRESH_TTL);
            } else {
    ...
    protected boolean multiDeviceCheck() {
        Long authId = LoginUtil.getLoginUserId();
        String token = LoginUtil.getToken();
        String key = Constants.REDIS_TOKEN + authId + "admin";
