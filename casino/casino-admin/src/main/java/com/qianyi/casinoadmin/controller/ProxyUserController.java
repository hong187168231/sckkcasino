...
public class ProxyUserController {
    ...
    public ResponseEntity<ProxyUserVo> findProxyUser(Integer pageSize, Integer pageCode, Integer proxyRole,
        Integer userFlag, Integer tag, String userName, Integer isDelete, Long companyId,
        ...
        if (proxyUserList != null && proxyUserList.size() > 0) {
            ...
            List<Long> secondProxyIds =
                proxyUserList.stream().filter(item -> item.getProxyRole() == CommonConst.NUMBER_3)
                    .map(ProxyUser::getSecondProxy).collect(Collectors.toList());
                ...
                proxyUserList.stream().forEach(u -> {
                    ...
                    proxyCommissions.stream().forEach(proxyCommission -> {
                        if (u.getProxyRole() == CommonConst.NUMBER_3
                            && u.getId().equals(proxyCommission.getProxyUserId())) {
                            String commissionRatio = MessageFormat.format(messageUtil.get(CommonConst.THIRD_FORMAT),
                                proxyCommission.getFirstCommission() == null ? "0"
                                    : proxyCommission.getFirstCommission().multiply(CommonConst.BIGDECIMAL_100),
                                proxyCommission.getSecondCommission() == null ? "0"
                                    : proxyCommission.getSecondCommission().multiply(CommonConst.BIGDECIMAL_100),
                                proxyCommission.getThirdCommission() == null ? "0"
                                    : proxyCommission.getThirdCommission().multiply(CommonConst.BIGDECIMAL_100));
                            proxyUserVo.setCommissionRatio(commissionRatio);
                        }
                        if (u.getProxyRole() == CommonConst.NUMBER_2
                            && u.getId().equals(proxyCommission.getProxyUserId())) {
                            String commissionRatio = MessageFormat.format(messageUtil.get(CommonConst.SECOND_FORMAT),
                                proxyCommission.getFirstCommission() == null ? "0"
                                    : proxyCommission.getFirstCommission().multiply(CommonConst.BIGDECIMAL_100));
                            proxyUserVo.setCommissionRatio(commissionRatio);
                        }
    ...
    public ResponseEntity saveProxyUser(String userName, String nickName, Long id, Integer tag, Long companyId) {
        ...
        if (tag == CommonConst.NUMBER_1) {
            ...
            if (LoginUtil.checkNull(proxy) || proxy.getIsDelete() == CommonConst.NUMBER_2
                || proxy.getUserFlag() == CommonConst.NUMBER_2) {
    ...
    private void createrProxyCommission(ProxyUser saveProxyUser, Integer proxyRole) {
        ...
        if (proxyRole == CommonConst.NUMBER_3) {
            ...
            proxyCommission
                .setFirstCommission((secondCommission == null || secondCommission.getFirstCommission() == null)
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "proxyRole", value = "proxyRole(区域：1 基层：2)", required = true),})
    @NoAuthorization
    ...
    public ResponseEntity<ProxyUser> findProxyUserList(Long id) {
        ...
        proxyUser.setIsDelete(CommonConst.NUMBER_1);
        List<ProxyUser> proxyUserList = proxyUserService.findProxyUserList(proxyUser);
        proxyUserList = proxyUserList == null ? new ArrayList<>()
            : proxyUserList.stream().filter(PUser -> !PUser.getId().equals(id)).collect(Collectors.toList());
        return ResponseUtil.success(proxyUserList);
    }
    ...
    @ApiOperation("转移会员")
    @ApiImplicitParams({@ApiImplicitParam(name = "id", value = "被转移者id(选中列id)", required = true),
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "id", value = "选中列id", required = true),})
    @NoAuthorization
    ...
    public ResponseEntity<ProxyUser> getTransferProxyUp(Long id) {
        ...
        proxyUser.setFirstProxy(byId.getFirstProxy());
        List<ProxyUser> proxyUserList = proxyUserService.findProxyUserList(proxyUser);
        proxyUserList = proxyUserList.stream().filter(proxy -> !proxy.getId().equals(byId.getSecondProxy()))
