        ...
        try {
            ...
            gameRecords.stream().forEach(item -> {
                ...
                if (LoginUtil.checkNull(user) || LoginUtil.checkNull(user.getFirstProxy())) {
                    userRunningWaterService.updateKey(userId, format, validbet, BigDecimal.ZERO, CommonConst.LONG_0,
    ...
    public void shareProfitChange(String format, String startDate, String endDate) {
        try {
            ...
            sumAmount.stream().forEach(item -> {
                ...
                User user = userService.findById(userId);
                if (LoginUtil.checkNull(user) || LoginUtil.checkNull(user.getFirstProxy())) {
                    userRunningWaterService.updateKey(userId, format, BigDecimal.ZERO, amount, CommonConst.LONG_0,
                        CommonConst.LONG_0, CommonConst.LONG_0);
                } else {
                    userRunningWaterService.updateKey(userId, format, BigDecimal.ZERO, amount, user.getFirstProxy(),
                        user.getSecondProxy(), user.getThirdProxy());
                }
            });
            sumAmount.clear();
