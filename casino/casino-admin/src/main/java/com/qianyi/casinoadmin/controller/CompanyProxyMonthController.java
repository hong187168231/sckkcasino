...
public class CompanyProxyMonthController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "pageSize", value = "每页大小(默认10条)", required = false),
        ...
        @ApiImplicitParam(name = "companyId", value = "所属公司", required = false),
        @ApiImplicitParam(name = "startDate", value = "起始时间", required = false),
        @ApiImplicitParam(name = "endDate", value = "结束时间", required = false),})
    public ResponseEntity<CompanyProxyMonthVo> find(Integer pageSize, Integer pageCode, Integer proxyRole, Integer tag,
        String userName, Long companyId, String startDate, String endDate) {
        ...
        proxyUser.setCompanyId(companyId);
        ...
        Set<Long> companyIds =
            proxyUserPage.getContent().stream().map(ProxyUser::getCompanyId).collect(Collectors.toSet());
        List<CompanyManagement> companyManagementList = companyManagementService.findCompanyManagementList(companyIds);
        ...
        List<CompanyProxyMonth> companyProxyMonths =
            companyProxyMonthService.findCompanyProxyMonths(proxyUserId, companyProxyMonth, startDate, endDate);
        ...
        // }
        Map<Long, List<CompanyProxyMonth>> firstMap =
            companyProxyMonths.stream().collect(Collectors.groupingBy(CompanyProxyMonth::getUserId));
        ...
        if (startDate.equals(endDate)) {
            proxyUserPage.getContent().forEach(proxy -> {
                ...
                companyProxyMonthVo.setCompanyName(maps.get(proxy.getCompanyId()));
                ...
                if (!LoginUtil.checkNull(proxyHomes) && proxyHomes.size() > CommonConst.NUMBER_0) {
                    companyProxyMonthVo
                        .setPlayerNum(proxyHomes.stream().mapToInt(CompanyProxyMonth::getPlayerNum).sum());
                    companyProxyMonthVo.setGroupBetAmount(proxyHomes.stream().map(CompanyProxyMonth::getGroupBetAmount)
                        .reduce(BigDecimal.ZERO, BigDecimal::add));
                    companyProxyMonthVo.setGroupTotalprofit(proxyHomes.stream()
                        .map(CompanyProxyMonth::getGroupTotalprofit).reduce(BigDecimal.ZERO, BigDecimal::add));
                    companyProxyMonthVo.setProfitAmount(proxyHomes.stream().map(CompanyProxyMonth::getProfitAmount)
                        .reduce(BigDecimal.ZERO, BigDecimal::add));
                    // 抽点金额
                    if (proxy.getProxyRole() == CommonConst.NUMBER_3) {
                        BigDecimal sum =
                            proxyHomes.stream().map(CompanyProxyMonth::getExtractPointsAmount).filter(Objects::nonNull)
                                .reduce(BigDecimal.ZERO, BigDecimal::add);
                        companyProxyMonthVo.setExtractPointsAmount(sum);
                    }
                    List<Integer> collect =
                        proxyHomes.stream().map(CompanyProxyMonth::getSettleStatus).collect(Collectors.toList());
                    ...
                    companyProxyMonthVo.setUpdateTime(companyProxyMonthVo.getSettleStatus() == CommonConst.NUMBER_0
                        ? null : proxyDetail.getUpdateTime());
                } else {
                    if (companyProxyMonthVo.getProxyRole() == CommonConst.NUMBER_3) {
    ...
    @GetMapping("/findSum")
    @ApiImplicitParams({@ApiImplicitParam(name = "proxyRole", value = "代理级别1：总代理 2：区域代理 3：基层代理", required = false),
        @ApiImplicitParam(name = "userName", value = "账号", required = false),
        @ApiImplicitParam(name = "tag", value = "1：含下级 0：不包含", required = false),
        @ApiImplicitParam(name = "companyId", value = "所属公司", required = false),
        @ApiImplicitParam(name = "startDate", value = "起始时间", required = false),
        @ApiImplicitParam(name = "endDate", value = "结束时间", required = false),})
    @NoAuthentication
    public ResponseEntity<CompanyProxyMonthVo> find(Integer proxyRole, Integer tag, String userName, Long companyId,
        ...
        List<ProxyUser> proxyUserList = proxyUserService.findProxyUserList(proxyUser, null, null);
        if (CollUtil.isEmpty(proxyUserList)) {
            return ResponseUtil.success(new CompanyProxyMonthVo());
        }
        List<Long> proxyUserId = proxyUserList.stream().map(ProxyUser::getId).collect(Collectors.toList());
        List<CompanyProxyMonth> companyProxyMonths =
            companyProxyMonthService.findCompanyProxyMonths(proxyUserId, companyProxyMonth, startDate, endDate);
        CompanyProxyMonthVo companyProxyMonthVo = new CompanyProxyMonthVo();
        companyProxyMonthVo.setProfitAmount(companyProxyMonths.stream().map(CompanyProxyMonth::getProfitAmount)
            .reduce(BigDecimal.ZERO, BigDecimal::add));
        companyProxyMonthVo
            .setExtractPointsAmount(companyProxyMonths.stream().map(CompanyProxyMonth::getExtractPointsAmount)
                .filter(Objects::nonNull).reduce(BigDecimal.ZERO, BigDecimal::add));
        if (Objects.isNull(proxyRole) || proxyRole == CommonConst.NUMBER_1){
            companyProxyMonths = companyProxyMonths.stream().filter(companyProxy -> companyProxy.getProxyRole() == CommonConst.NUMBER_1).collect(Collectors.toList());
        }else if (proxyRole == CommonConst.NUMBER_2){
            companyProxyMonths = companyProxyMonths.stream().filter(companyProxy -> companyProxy.getProxyRole() == CommonConst.NUMBER_2).collect(Collectors.toList());
        }else {
            companyProxyMonths = companyProxyMonths.stream().filter(companyProxy -> companyProxy.getProxyRole() == CommonConst.NUMBER_3).collect(Collectors.toList());
        }
        companyProxyMonthVo.setGroupBetAmount(companyProxyMonths.stream().map(CompanyProxyMonth::getGroupBetAmount)
            .reduce(BigDecimal.ZERO, BigDecimal::add));
        companyProxyMonthVo.setGroupTotalprofit(companyProxyMonths.stream().map(CompanyProxyMonth::getGroupTotalprofit)
            .reduce(BigDecimal.ZERO, BigDecimal::add));
        companyProxyMonthVo.setPlayerNum(companyProxyMonths.stream().mapToInt(CompanyProxyMonth::getPlayerNum).sum());
        return ResponseUtil.success(companyProxyMonthVo);
    }

    @ApiOperation("结清佣金")
    @GetMapping("/update")
    @ApiImplicitParams({@ApiImplicitParam(name = "id", value = "当前列id", required = true),})
    @NoAuthorization
    ...
    // return;
    // BigDecimal groupBetAmount = list.stream().map(CompanyProxyMonth::getGroupBetAmount).reduce(BigDecimal.ZERO,
    // BigDecimal::add);
    // BigDecimal groupTotalprofit = list.stream().map(CompanyProxyMonth::getGroupTotalprofit).reduce(BigDecimal.ZERO,
    // BigDecimal::add);
    // BigDecimal profitAmount = list.stream().map(CompanyProxyMonth::getProfitAmount).reduce(BigDecimal.ZERO,
    // BigDecimal::add);
    ...
    // }
    //
    // private Boolean findCompanyProxyMonths(Integer tag, String userName, CompanyProxyMonth companyProxyMonth, Sort
    // sort, List<CompanyProxyMonth> companyProxyMonthList){
    // List<CompanyProxyMonth> companyProxyMonths;
    // if (!LoginUtil.checkNull(userName)){
    // if (LoginUtil.checkNull(tag)){
    // return true;
    ...
    // companyProxyMonth.setSecondProxy(byUserName.getId());
    // companyProxyMonths = companyProxyMonthService.findAll(companyProxyMonth, sort);
    // companyProxyMonths = companyProxyMonths.stream().filter(item -> (item.getProxyRole() !=
    // CommonConst.NUMBER_1)).collect(Collectors.toList());
    // }else {
    // companyProxyMonth.setThirdProxy(byUserName.getId());
    // companyProxyMonths = companyProxyMonthService.findAll(companyProxyMonth, sort);
    // companyProxyMonths = companyProxyMonths.stream().filter(item -> (item.getProxyRole() ==
    // CommonConst.NUMBER_3)).collect(Collectors.toList());
    // }
    // }else {
    // companyProxyMonth.setUserId(byUserName.getId());
    // companyProxyMonths = companyProxyMonthService.findAll(companyProxyMonth, sort);
