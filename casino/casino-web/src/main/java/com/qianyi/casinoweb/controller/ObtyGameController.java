...
public class ObtyGameController {
    ...
    public ResponseEntity<String> openGame(HttpServletRequest request) {
        ...
        ResponseEntity response =
            thirdGameBusiness.checkPlatformAndGameStatus(Constants.PLATFORM_OB, Constants.PLATFORM_OBTY);
...
//            userMoneyLock.lock(RedisKeyUtil.LOCK_TIME, TimeUnit.SECONDS);
            ...
            if (userCenterMoney.compareTo(BigDecimal.ZERO) == 1) {
                ...
                // 加点
                PublicObtyApi.ResponseEntity transfer =
                    obtyApi.transfer(third.getObtyAccount(), 1, userCenterMoney, orderNo);
                if (transfer == null) {
                    log.error("userId:{},account:{},money:{},进OB体育游戏加点失败,远程请求异常", third.getUserId(), user.getAccount(),
                        userCenterMoney);
                    // 异步记录错误订单并重试补偿
                    errorOrderService.syncSaveErrorOrder(third.getObtyAccount(), user.getId(), user.getAccount(),
                        orderNo, userCenterMoney, AccountChangeEnum.OBTY_IN, Constants.PLATFORM_OBTY);
                    return ResponseUtil.custom("服务器异常,请重新操作");
                }
                ...
                thirdGameBusiness.inSaveAccountChange(authId, userCenterMoney, userMoney.getMoney(),
                    userMoney.getMoney().subtract(userCenterMoney), 0, orderNo, "自动转入OB体育", Constants.PLATFORM_OBTY,
                    AccountChangeEnum.OBTY_IN);
            }
        } catch (Exception e) {
            log.error("OBTY上分出现异常userId{} {}", authId, e.getMessage());
            return ResponseUtil.custom("服务器异常,请重新操作");
        } finally {
            // 释放锁
