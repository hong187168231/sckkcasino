...
public class SupplementController {
    ...
    @GetMapping("/ae")
    @ApiOperation("AE平台手动补单")
    @NoAuthentication
    @ApiImplicitParams({
            @ApiImplicitParam(name = "platform", value = "斗鸡.SV388,赛马.HORSEBOOK,电竞.E1SPORT", required = true),
            @ApiImplicitParam(name = "startTime", value = "开始时间,格式为:yyyy-MM-dd HH:mm:ss", required = true),
            @ApiImplicitParam(name = "endTime", value = "结束时间,格式为:yyyy-MM-dd HH:mm:ss", required = true),
    })
    public ResponseEntity aeSupplement(String platform, String startTime, String endTime) {
        boolean checkNull = CasinoWebUtil.checkNull(platform, startTime, endTime);
        if (checkNull) {
            return ResponseUtil.parameterNotNull();
        }
        ...
        try {
            ...
            long days = diff / (1000 * 60 * 60 * 24);
            if (days > 7) {
                return ResponseUtil.custom("仅允许补单7天内的数据");
            }
        } catch (ParseException e) {
            return ResponseUtil.custom("startTime或endTime时间格式填写错误,,格式为:yyyy-MM-dd HH:mm:ss");
        }
        getDateTimeAeReport(platform, startTime, endTime);
        return ResponseUtil.success();
    }

    private void getDateTimeAeReport(String platform, String startTime, String endTime) {
        SimpleDateFormat df = DateUtil.getSimpleDateFormat();
        String timeMsg = null;
        String endTimeNew = null;
        try {
            ...
            long endTimeNum = endDate.getTime();
            //每次最大拉取区间仅可设置为 1 小时
            if ((endTimeNum - startTimeNum) > 60 * 60 * 1000) {
                //下面开始时间前移2分钟，结束时间也要前移2分钟
                endTimeNew = getBeforeDateTime(df, startTime, 60);
            } else {
                endTimeNew = endTime;
            }
            timeMsg = startTime + "到" + endTimeNew;
            ...
            Date endDateTime = df.parse(endTimeNew);
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssXXX");
            String start = sdf.format(startDateTime);
            String end = sdf.format(endDateTime);
            JSONObject result = aeApi.getTransactionByTxTime(start, end, platform, null, null, null, null, null);
            //远程请求异常
            if (result == null) {
                log.error("{}AE游戏记录拉取失败,远程请求异常", timeMsg);
            } else {
                String status = result.getString("status");
                if (!PublicAeApi.SUCCESS_CODE.equals(status)) {
                    log.error("{}AE游戏记录拉取失败,result={}", timeMsg, result);
                } else {
            ...
            log.error("{}AE游戏记录拉取异常,msg={}", timeMsg, e.getMessage());
            e.printStackTrace();
        }
        if (!endTime.equals(endTimeNew)) {
            try {
            ...
            getDateTimeAeReport(platform, endTimeNew, endTime);
        }
    }
