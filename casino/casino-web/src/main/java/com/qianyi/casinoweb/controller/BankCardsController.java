...
public class BankCardsController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "id", value = "银行卡id", required = true),})
    public ResponseEntity deleteBankCardById(Long id) {
        ...
        RLock bankcardsLock = redisKeyUtil.getBankcardsLock(authId.toString());
        try {
            bankcardsLock.lock(RedisKeyUtil.LOCK_TIME, TimeUnit.SECONDS);
            ...
            log.error("web会员删除银行卡出现异常:{}", ex.getMessage());
            return ResponseUtil.custom("请重试一次");
        } finally {
            RedisKeyUtil.unlock(bankcardsLock);
        }
    ...
    public ResponseEntity setDefaultBankCardById(Long id) {
        ...
        try {
            ...
            log.error("web会员设置默认银行卡银行卡出现异常:{}", ex.getMessage());
            return ResponseUtil.custom("请重试一次");
        } finally {
            RedisKeyUtil.unlock(bankcardsLock);
        }
