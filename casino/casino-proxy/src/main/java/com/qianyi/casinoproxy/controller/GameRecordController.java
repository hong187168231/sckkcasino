...
public class GameRecordController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "pageSize", value = "每页大小(默认10条)", required = false),
            ...
            @ApiImplicitParam(name = "endDate", value = "查询结束时间查询", required = false),})
    @NoAuthorization
    public ResponseEntity<GameRecordObtyVo> findOBZRGameRecordPage(Integer pageSize, Integer pageCode, String user,
                                                                   String orderNo, Integer outcome, String account, Integer tag,
                                                                   @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") Date startDate,
                                                                   @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") Date endDate) {
        Sort sort = Sort.by("id").descending();
        ...
        if (CasinoProxyUtil.setParameter(gameRecordObzr)){
            return ResponseUtil.custom(CommonConst.NETWORK_ANOMALY);
        }
        ...
        if (!ObjectUtils.isEmpty(startDate) && !ObjectUtils.isEmpty(endDate)) {
            ...
            if (CasinoProxyUtil.checkNull(tag) || tag == CommonConst.NUMBER_0) {
                gameRecordObzrPage =
                        gameRecordObzrService.findGameRecordPage(gameRecordObzr, pageable, startTime, endTime, null, null);
            } else {
                gameRecordObzrPage =
                        gameRecordObzrService.findGameRecordPage(gameRecordObzr, pageable, null, null, startTime, endTime);
            }
        } else {
            gameRecordObzrPage =
                    gameRecordObzrService.findGameRecordPage(gameRecordObzr, pageable, null, null, null, null);
        }
    ...
    public ResponseEntity<GameRecordAeVo> findSV388GameRecordPage(Integer pageSize, Integer pageCode, String platformTxId,
        ...
        if (CasinoProxyUtil.setParameter(gameRecordAe)){
            return ResponseUtil.custom(CommonConst.NETWORK_ANOMALY);
        }
